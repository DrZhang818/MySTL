...
struct decay_selector {
    using type = conditional_t<is_function_v<U>, add_pointer_t<U>, remove_cv_t<U>>;
};
...
template <typename T>
struct decay {
    using type = typename detail::decay_selector<remove_reference_t<T>>::type;
};
...
template <typename T>
struct is_integral : detail::in_integral_helper<remove_cv_t<T>> {};
...
template <typename T>
struct is_signed : detail::is_signed_helper<remove_cv_t<T>> {};

template <typename T>
inline constexpr bool is_signed_v = is_signed<T>::value;

template <typename T>
struct is_unsigned : detail::is_unsigned_helper<remove_cv_t<T>> {};

template <typename T>
inline constexpr bool is_unsigned_v = is_unsigned<T>::value;

template <typename T>
struct is_null_pointer : is_same<remove_cv_t<T>, mystl::nullptr_t> {};
...
template <typename T>
struct is_scalar : disjunction<is_arithmetic<T>, is_pointer<T>, is_member_pointer<T>, is_enum<T>,
                               is_null_pointer<T>> {};
...
namespace detail {
template <typename T>
concept can_destruct = requires(T& t) { t.~T(); };
}  // namespace detail

template <typename T>
struct is_destructible : mystl::false_type {};

template <typename T>
    requires mystl::is_reference_v<T>
...
template <typename T, typename U>
struct apply_cv<const T, U> {
    using type = const U;
};
template <typename T, typename U>
struct apply_cv<volatile T, U> {
    using type = volatile U;
};
template <typename T, typename U>
struct apply_cv<const volatile T, U> {
...
template <>
struct make_unsigned_helper<wchar_t> {
    using type = conditional_t<
        sizeof(wchar_t) == sizeof(unsigned short), unsigned short,
        conditional_t<sizeof(wchar_t) == sizeof(unsigned int), unsigned int, unsigned long long>>;
};
...
template <>
struct make_signed_helper<char8_t> {
    using type = signed char;
};
#endif
template <>
struct make_signed_helper<char16_t> {
    using type = conditional_t<sizeof(char16_t) == sizeof(short), short, int>;
};
template <>
struct make_signed_helper<char32_t> {
    using type = conditional_t<sizeof(char32_t) == sizeof(int), int, long>;
};
template <>
struct make_signed_helper<wchar_t> {
    using type = conditional_t<sizeof(wchar_t) == sizeof(short), short,
                               conditional_t<sizeof(wchar_t) == sizeof(int), int, long long>>;
};
